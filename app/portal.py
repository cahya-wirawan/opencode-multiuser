...
#oc-mu-portal .ocmu-menu{position:absolute;right:0;top:calc(100% + 7px);width:210px;border:1px solid rgba(0,0,0,.16);border-radius:10px;background:#fff;box-shadow:0 8px 28px rgba(0,0,0,.20);overflow:hidden}
...
(() => {
  ...
  stopForm?.addEventListener('submit', (e) => {
    if (!window.confirm('Stop this workspace and free its slot?')) e.preventDefault();
  });
  logoutForm?.addEventListener('submit', (e) => {
    if (!window.confirm('Logout and stop your running workspace(s)?')) e.preventDefault();
  });
  ...
  async function poll() {
    try {
      ...
      if (status) status.textContent = `Running · slot ${data.slot_id ?? '-'}`;
    } catch (_) {
...
def portal_markup(project_slug: str) -> str:
    ...
    return (
        ...
        '<a href="/_portal/dashboard">Dashboard</a>'
